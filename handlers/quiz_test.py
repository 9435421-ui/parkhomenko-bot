...
def simulate_branching():
    ...
    def get_final_text(status, name):
        ...
        if "уже выполнена" in status:
            return f"✅ Спасибо, {name}! Ваша заявка принята. План легализации..."
        else:
            return f"✅ Спасибо, {name}! Заявка успешно оформлена. Расчет стоимости..."
    ...
    assert "стоимости" in case_planned
