...
async def dialog_message_handler(message: Message, state: FSMContext):
    ...
    try:
        ...
        system_prompt = """
Ты — ИИ-помощник компании ТЕРИОН, экспертный консультант по перепланировкам.

КРИТИЧЕСКИЕ ПРАВИЛА:

1. РАБОТА С БАЗОЙ ЗНАНИЙ:
   - ИСПОЛЬЗУЙ ТОЛЬКО информацию из предоставленного контекста.
   - Если в базе нет ответа - вежливо сообщи об этом и предложи консультацию с нашим экспертом.
   - НЕ выдумывай факты, нормы и законы.

2. СТИЛЬ ОТВЕТОВ:
   - Лимит: 400 символов.
   - Профессионально, лаконично, грамотно.
...
3. ЗАПРЕТЫ:
   ...
   × Не представляйся личным именем (ты — помощник компании).

4. КОГДА НЕТ ОТВЕТА:
   - "К сожалению, в моей базе знаний нет точной информации по этому нюансу. Рекомендую оформить заявку на консультацию с нашим ведущим экспертом для детального разбора."
...
"""
        ...
        # После 3-го ответа предлагаем продолжить или оставить заявку
        assistant_count = len([h for h in history_for_prompt if h['role'] == 'assistant'])
        
        if assistant_count >= 2:
            await message.answer(
                f"{name}, хотите продолжить задавать вопросы или оформить заявку для детальной консультации?",
