"""
Обработчик диалогового режима с RAG (консультант ТЕРИОН)
"""
from aiogram import Router, F
from aiogram.types import Message, CallbackQuery, InlineKeyboardMarkup, InlineKeyboardButton, WebAppInfo
from aiogram.fsm.context import FSMContext
from database import db
from utils import yandex_gpt, kb
from keyboards import get_continue_or_menu_keyboard
from config import MINI_APP_URL

router = Router()


@router.callback_query(F.data == "mode:dialog")
async def start_dialog(callback: CallbackQuery, state: FSMContext):
    """Запуск диалогового режима"""
    user_id = callback.from_user.id
    
    # Устанавливаем режим диалога
    await db.update_user_state(user_id, mode="dialog")
    await state.set_state(None) # Очищаем FSM, так как работаем через user_state в БД
    
    user_data = await db.get_or_create_user(user_id)
    name = user_data.get('first_name', 'дорогой клиент')
    
    await callback.message.edit_text(
        f"💬 <b>Консультация с ИИ-экспертом</b>\n\n"
        f"{name}, я — ваш цифровой помощник по вопросам перепланировок. "
        f"Готов ответить на ваши вопросы, опираясь на обширную базу знаний "
        f"по законодательству РФ и многолетнему практическому опыту нашей компании.\n\n"
        f"<b>Опишите вашу ситуацию или задайте конкретный вопрос.</b>",
        parse_mode="HTML"
    )
    await callback.answer()


@router.message(F.text)
async def dialog_message_handler(message: Message, state: FSMContext):
    """Обработка сообщений в диалоговом режиме"""
    user_id = message.from_user.id
    db_state = await db.get_user_state(user_id)
    
    # Проверяем, что пользователь в режиме диалога
    if not db_state or db_state.get('mode') != 'dialog':
        return
    
    # Проверка лимита вопросов (5 вопросов на сессию)
    dialog_count = db_state.get('dialog_count', 0)
    if dialog_count >= 5:
        await message.answer(
            "📍 <b>Лимит вопросов исчерпан.</b>\n\n"
            "Для получения полной экспертной консультации или расчета стоимости, "
            "пожалуйста, оформите заявку или воспользуйтесь нашим Mini App.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📝 Оформить заявку", callback_data="mode:quiz")],
                [InlineKeyboardButton(text="🌐 Перейти в Mini App", web_app=WebAppInfo(url=MINI_APP_URL))],
                [InlineKeyboardButton(text="◀️ В меню", callback_data="back_to_menu")]
            ]),
            parse_mode="HTML"
        )
        return

    user_query = message.text.strip()
    name = db_state.get('name', '')

    # Увеличиваем счетчик вопросов
    await db.increment_dialog_count(user_id)
    
    # Сохраняем сообщение пользователя в историю
    await db.add_dialog_message(user_id, role="user", message=user_query)
    
    # Получаем контекст из базы знаний через RAG
    rag_context = await kb.get_context(user_query, max_chunks=3, context_size=800)
    
    # Получаем историю диалога
    dialog_history = await db.get_dialog_history(user_id, limit=10)
    
    # Формируем историю для промпта
    history_for_prompt = []
    for msg in dialog_history:
        history_for_prompt.append({
            'role': msg['role'],
            'text': msg['message']
        })
    
    # Проверка на триггер-слова (связь со специалистом)
    trigger_words = [
        'специалист', 'менеджер', 'человек', 'живой', 'соединить',
        'связаться', 'заказать', 'консультация', 'записаться'
    ]
    
    if any(word in user_query.lower() for word in trigger_words):
        from handlers.quiz import QuizOrder

        # Переключаем в режим квиза
        await db.update_user_state(user_id, mode="quiz")
        
        await message.answer(
            f"{name}, отлично! Давайте оформим заявку для связи со специалистом.\n\n"
            f"Я задам несколько вопросов, чтобы наш эксперт смог подготовить "
            f"предварительную консультацию.",
            parse_mode="HTML"
        )
        
        await state.set_state(QuizOrder.city)

        await message.answer("📋 <b>Начинаем квиз</b>\n\n1. Укажите город / населенный пункт.", parse_mode="HTML")
        return
    
    # Генерируем ответ через YandexGPT с RAG
    try:
        # Формируем системный промпт для консультанта
        system_prompt = """
Ты — Антон, ИИ-помощник компании ТЕРИОН, экспертный консультант по перепланировкам. Работаешь от имени ведущего эксперта.

КРИТИЧЕСКИЕ ПРАВИЛА:

1. РАБОТА С БАЗОЙ ЗНАНИЙ:
   - ИСПОЛЬЗУЙ ТОЛЬКО информацию из предоставленного контекста.
   - Если в базе нет ответа - вежливо сообщи об этом и предложи консультацию.
   - НЕ выдумывай факты, нормы и законы.

2. СТИЛЬ ОТВЕТОВ:
   - СТРОГИЙ ЛИМИТ: 500 знаков.
   - Твоя цель: разогреть интерес и направить в Mini App или на заявку.
   - Обязательно ссылайся на статьи ЖК РФ, если они есть в контексте.

3. ЗАПРЕТЫ:
   × Никаких полных консультаций "под ключ" в чате.
   × Не называй конкретную стоимость услуг или работ.
   × Не давай гарантий 100% согласования.

4. КОГДА НЕТ ОТВЕТА:
   - "В моей базе знаний нет точной информации по этому нюансу. Рекомендую оформить заявку для детального разбора нашим экспертом."

КОНТЕКСТ ИЗ БАЗЫ ЗНАНИЙ:
"""
        
        # Формируем полный промпт
        user_prompt = f"{system_prompt}\n\n{rag_context}\n\nВОПРОС КЛИЕНТА:\n{user_query}"
        
        # Генерируем ответ
        response = await yandex_gpt.generate_response(
            user_prompt=user_prompt,
            temperature=0.2,
            max_tokens=300,
            model="yandexgpt"
        )
        
        # Сохраняем ответ в историю
        await db.add_dialog_message(user_id, role="assistant", message=response)
        
        # Добавляем подпись ИИ-помощника
        response_with_label = f"🤖 <b>ИИ-помощник:</b>\n\n{response}"
        
        # Клавиатура CTA
        cta_markup = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📝 Оформить заявку", callback_data="mode:quiz")],
            [InlineKeyboardButton(text="🌐 Mini App: Аналитика объекта", web_app=WebAppInfo(url=MINI_APP_URL))],
            [InlineKeyboardButton(text="◀️ В меню", callback_data="back_to_menu")]
        ])

        # Отправляем ответ пользователю
        await message.answer(response_with_label, reply_markup=cta_markup, parse_mode="HTML")
    
    except Exception as e:
        print(f"❌ Ошибка генерации ответа: {e}")
        await message.answer(
            "Извините, произошла ошибка при формировании ответа. "
            "Попробуйте переформулировать вопрос или свяжитесь со специалистом.",
            reply_markup=get_continue_or_menu_keyboard()
        )
