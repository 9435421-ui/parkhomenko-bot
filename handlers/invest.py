...
def register_handlers(dp: Dispatcher):
    ...
    @dp.callback_query_handler(lambda c: c.data == "mode:invest")
    async def start_invest_mode(callback: types.CallbackQuery, state: FSMContext):
    ...
    async def handle_web_app_data(message: types.Message):
        ...
        try:
            ...
            username = user.username if user.username else 'id'+str(user.id)
            summary = (
    ...
    @dp.message_handler()
    async def invest_handler(message: types.Message, state: FSMContext):
        ...
        # Если пользователь пишет текст в этом режиме
        await message.answer(
