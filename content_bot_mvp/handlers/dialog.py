"""
Обработчик диалогового режима с RAG (консультант Антон)
"""
from aiogram import Router, F
from aiogram.types import Message, CallbackQuery
from database import db
from utils import yandex_gpt, kb
from keyboards import get_continue_or_menu_keyboard

router = Router()


@router.callback_query(F.data == "mode:dialog")
async def start_dialog(callback: CallbackQuery):
    """Запуск диалогового режима"""
    user_id = callback.from_user.id
    
    # Устанавливаем режим диалога
    await db.update_user_state(user_id, mode="dialog")
    
    state = await db.get_user_state(user_id)
    name = state.get('name', 'дорогой клиент')
    
    await callback.message.edit_text(
        f"💬 <b>Консультация с Антоном</b>\n\n"
        f"{name}, я - ИИ-помощник эксперта Пархоменко Юлии Владимировны по перепланировкам. "
        f"Готов ответить на ваши вопросы, опираясь на базу знаний из 83 документов "
        f"по законодательству РФ и практическому опыту.\n\n"
        f"<b>Опишите вашу ситуацию или задайте конкретный вопрос.</b>",
        parse_mode="HTML"
    )
    await callback.answer()


@router.message(F.text)
async def dialog_message_handler(message: Message):
    """Обработка сообщений в диалоговом режиме"""
    user_id = message.from_user.id
    state = await db.get_user_state(user_id)
    
    # Проверяем, что пользователь в режиме диалога
    if not state or state.get('mode') != 'dialog':
        return
    
    user_query = message.text.strip()
    name = state.get('name', '')
    
    # Сохраняем сообщение пользователя в историю
    await db.add_dialog_message(user_id, role="user", message=user_query)
    
    # Получаем контекст из базы знаний через RAG
    rag_context = await kb.get_context(user_query, max_chunks=3, context_size=800)
    
    # Получаем историю диалога
    dialog_history = await db.get_dialog_history(user_id, limit=10)
    
    # Формируем историю для промпта
    history_for_prompt = []
    for msg in dialog_history:
        history_for_prompt.append({
            'role': msg['role'],
            'text': msg['message']
        })
    
    # Проверка на триггер-слова (связь со специалистом)
    trigger_words = [
        'специалист', 'менеджер', 'человек', 'живой', 'соединить',
        'связаться', 'заказать', 'консультация', 'записаться'
    ]
    
    if any(word in user_query.lower() for word in trigger_words):
        await db.update_user_state(user_id, mode="quiz", quiz_step=1)
        
        await message.answer(
            f"{name}, отлично! Давайте оформим заявку для связи со специалистом.\n\n"
            f"Я задам несколько вопросов, чтобы наш эксперт смог подготовить "
            f"предварительную консультацию.",
            parse_mode="HTML"
        )
        
        # Импортируем и вызываем квиз
        from handlers.quiz import get_city_keyboard
        
        await message.answer(
            "📝 <b>Заявка на консультацию</b>\n\n"
            "<b>Вопрос 1 из 8:</b> В каком городе находится объект?",
            reply_markup=get_city_keyboard(),
            parse_mode="HTML"
        )
        return
    
    # Генерируем ответ через YandexGPT с RAG
    try:
        # Формируем системный промпт для консультанта
        system_prompt = """
Ты — Антон, ИИ-помощник эксперта Пархоменко Юлии Владимировны по перепланировкам.

КРИТИЧЕСКИЕ ПРАВИЛА:

1. РАБОТА С БАЗОЙ ЗНАНИЙ (83 документа):
   - ИСПОЛЬЗУЙ ТОЛЬКО информацию из контекста ниже
   - Если в базе нет ответа - скажи честно и предложи консультацию со специалистом
   - НЕ выдумывай факты и нормы

2. СТИЛЬ ОТВЕТОВ:
   - Лимит: 350 символов (2-3 предложения)
   - Профессионально, со ссылками на ЖК РФ
   - Конкретно и по делу

3. ССЫЛКИ НА ЗАКОНЫ:
   - Упоминай статьи ЖК РФ, СП, СанПиН когда они есть в контексте
   - Формат: "Согласно ст. 26 ЖК РФ..."

4. КОГДА НЕТ ОТВЕТА:
   - "К сожалению, в базе знаний нет точной информации по вашему вопросу. 
     Рекомендую записаться на консультацию к Юлии Владимировне для детального разбора."

5. НЕ ДЕЛАЙ:
   × Не называй конкретные суммы
   × Не гарантируй 100% результат
   × Не давай инструкции по стройке

КОНТЕКСТ ИЗ БАЗЫ ЗНАНИЙ:
"""
        
        # Формируем полный промпт
        user_prompt = f"{system_prompt}\n\n{rag_context}\n\nВОПРОС КЛИЕНТА:\n{user_query}"
        
        # Генерируем ответ
        response = await yandex_gpt.generate_response(
            user_prompt=user_prompt,
            temperature=0.2,
            max_tokens=300,
            model="yandexgpt"
        )
        
        # Сохраняем ответ в историю
        await db.add_dialog_message(user_id, role="assistant", message=response)
        
        # Отправляем ответ пользователю
        await message.answer(response, parse_mode="HTML")
        
        # После 3-го ответа предлагаем продолжить или оставить заявку
        assistant_count = len([h for h in history_for_prompt if h['role'] == 'assistant'])
        
        if assistant_count >= 2:
            await message.answer(
                f"{name}, хотите продолжить задавать вопросы или оформить заявку для детальной консультации?",
                reply_markup=get_continue_or_menu_keyboard()
            )
    
    except Exception as e:
        print(f"❌ Ошибка генерации ответа: {e}")
        await message.answer(
            "Извините, произошла ошибка при формировании ответа. "
            "Попробуйте переформулировать вопрос или свяжитесь со специалистом.",
            reply_markup=get_continue_or_menu_keyboard()
        )
