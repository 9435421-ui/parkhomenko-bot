...
class ContentGenerator:
    ...
    async def generate_post_text(self, theme: str, post_type: str) -> str:
        """Генерирует текст поста с учетом ToV ТЕРИОН"""
        prompt = f"""
        Ты — экспертный копирайтер бренда «ТЕРИОН» (эксперты по перепланировкам).
        Напиши пост на тему: {theme}
        Тип поста: {post_type}

        Требования:
        1. Стиль: экспертный, но доступный, без использования конкретных имен (только «наши эксперты», «специалисты ТЕРИОН»).
        2. Структура: цепляющий заголовок, основная часть с пользой, четкий призыв к действию (CTA).
        3. CTA должен вести в бота @TerionProjectBot.
