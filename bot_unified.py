...
KNOWLEDGE_DIR = "terion_kb/knowledge_base"
...
PRIVACY_POLICY_TEXT = (
    ...
    "Я — Антон, ИИ-консультант и личный помощник эксперта компании ТЕРИОН. "
...
AI_INTRO_TEXT = (
    ...
    "подготавливая их к экспертной консультации с экспертом компании ТЕРИОН.\n\n"
    "⚠️ Важно: я — искусственный интеллект. Мои рекомендации носят информационный характер. "
    "Эксперт компании ТЕРИОН даст вам полную информацию после анализа документов."
)
...
def show_privacy_consent(chat_id: int):
    """Показывает сообщение с кнопками согласия на обработку данных"""
    text = (
        "📋 **Добро пожаловать в сервис консультаций по перепланировке ТЕРИОН!**\n\n"
        "Перед началом работы необходимо:\n"
        "✅ Согласие на обработку персональных данных\n"
        "✅ Согласие на принятие условий Пользовательского соглашения\n\n"
        ...
        "Нажимая кнопку «✅ Принимаю», вы подтверждаете согласие с условиями."
    )
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("✅ Принимаю", callback_data="consent_accept"))
    markup.add(types.InlineKeyboardButton("❌ Не принимаю", callback_data="consent_decline"))
...
def contact_handler(message):
    ...
    bot.send_message(chat_id, "✅ Номер получен, спасибо!", reply_markup=types.ReplyKeyboardRemove())

    # После получения телефона завершаем процесс
    save_lead_and_notify(chat_id, scenario="Контакт получен")
    send_final_success_msg(chat_id)

def is_working_hours():
    """Проверяет, рабочее ли сейчас время (10:00 - 20:00 МСК)"""
    from datetime import datetime, timedelta, timezone
    # МСК - это UTC+3
    now_msk = datetime.now(timezone(timedelta(hours=3)))
    return 10 <= now_msk.hour < 20

def send_checklist(chat_id: int):
    """Отправляет чек-лист документов пользователю"""
    checklist_text = (
        "✅ **Ваш запрос передан эксперту компании ТЕРИОН.**\n\n"
        "📋 **Что понадобится для согласования:**\n"
        "1️⃣ **Выписка из ЕГРН** (можно заказать онлайн).\n"
        "2️⃣ **Техпаспорт БТИ** (или поэтажный план с экспликация).\n\n"
        "📞 Эксперт свяжется в рабочее время (10:00–20:00 МСК).\n\n"
        "🎁 [Скачать полный чек-лист PDF](https://remodeling-expert.ru/checklists/terion_start_checklist.pdf)"
    )
    bot.send_message(chat_id, checklist_text, parse_mode="Markdown")

def send_final_success_msg(chat_id: int):
    """Отправляет финальное сообщение об успехе с учетом рабочих часов"""
    state = get_user_state(chat_id)
    ...
    if not state.phone:
        ...
        bot.send_message(
            chat_id,
            "✅ Данные приняты! Чтобы мы могли связаться с вами и отправить чек-лист, пожалуйста, нажмите кнопку ниже и поделитесь номером телефона:",
            reply_markup=markup
        )
        return

    if is_working_hours():
        timing_info = "в ближайшее время"
    else:
        timing_info = "ежедневно с 10:00 до 20:00 по Москве"

    final_msg = (
        "✅ **Спасибо! Ваша заявка принята.**\n\n"
        f"Команда **ТЕРИОН** свяжется с вами {timing_info} по номеру {state.phone} для обсуждения деталей и предварительного расчёта.\n\n"
    ...
    bot.send_message(chat_id, final_msg, reply_markup=types.ReplyKeyboardRemove())
...
def route_user(user_id):
    ...
    if module == "quiz":
        ...
        state.mode = BotModes.EXPERT
        state.quiz_step = 1
        bot.send_message(user_id, "⚖️ **Экспертиза объекта**\n\nКратко опишите, что вы планируете сделать (например: перенос санузла, объединение кухни):", parse_mode="Markdown")
    elif module == "price":
        state.mode = BotModes.PRICE
        state.quiz_step = 1
        bot.send_message(user_id, "💵 **Расчет стоимости услуг**\n\nУкажите тип вашего дома (панельный, кирпичный, монолит):", parse_mode="Markdown")
    elif module == "ask":
...
def brand_advocate_handler(message):
    ...
    if any(kw in text for kw in keywords):
        ...
        system_prompt = """
Роль: Ты — Антон, гениальный ИИ-консультант компании ТЕРИОН.
ВАЖНО: Обязательно начни ответ с фразы или включи её: «Я — ИИ-помощник компании ТЕРИОН».
...
- Предложи написать тебе в личные сообщения для детального разбора или перейти в @TerionProjectBot.
""".strip()
...
def start_handler(message):
    ...
    if len(args) > 1:
        ...
        elif "expert" in param: state.target_module = "expert"
        elif "price" in param: state.target_module = "price"
...
def main_menu_handler(message):
    ...
    if message.text == "📝 Квиз":
        ...
        bot.send_message(user_id, "📞 **Наши контакты:**\n\n👤 Эксперт: Эксперт компании ТЕРИОН\n🌐 Сайт: [terion-expert.ru](https://terion-expert.ru)\n📱 Телефон: +7 (926) 336-72-82", parse_mode="Markdown")

@bot.message_handler(func=lambda m: get_user_state(m.chat.id).mode == BotModes.EXPERT, content_types=["text"])
def expert_handler(message):
    if message.from_user.is_bot: return
    chat_id = message.chat.id
    state = get_user_state(chat_id)
    ...
    if state.quiz_step == 1:
        state.change_plan = text
        state.quiz_step = 2
        bot.send_message(chat_id, "🏢 Есть ли несущие стены в зоне планируемых изменений? (да/нет/не знаю)")
    elif state.quiz_step == 2:
        state.bti_status = f"Несущие стены: {text}"
        state.quiz_step = 3
        bot.send_message(chat_id, "💦 Планируете ли вы перенос или расширение мокрых зон (санузел, кухня)?")
    elif state.quiz_step == 3:
        state.extra_contact = f"Мокрые зоны: {text}"
        state.quiz_step = 4
        # Завершение
        save_lead_and_notify(chat_id, scenario="Экспертиза")

        # Предварительный вердикт согласно ТЗ
        res = (
            "✅ **Задача предварительно решаема!**\n\n"
            "Я передал ваш запрос эксперту компании **ТЕРИОН** для подготовки детального заключения."
        )
...
@bot.message_handler(func=lambda m: get_user_state(m.chat.id).mode == BotModes.PRICE, content_types=["text"])
def price_handler(message):
    ...
    if state.quiz_step == 1:
        state.bti_status = f"Тип дома: {text}"
        state.quiz_step = 2
        bot.send_message(chat_id, "📐 Укажите общую площадь объекта (кв.м):")
    elif state.quiz_step == 2:
        state.city = f"Площадь: {text} кв.м"
        state.quiz_step = 3
        bot.send_message(chat_id, "🛠️ Какие именно работы планируются? (например: перенос кухни, объединение санузла)")
    elif state.quiz_step == 3:
        state.change_plan = text
        state.quiz_step = 4
        # Завершение
        save_lead_and_notify(chat_id, scenario="Калькулятор цен")

        # Предварительный вердикт согласно ТЗ
        res = (
            "✅ **Данные получены!**\n\n"
            "Смету рассчитает эксперт компании **ТЕРИОН** после изучения документов. Команда свяжется с вами для уточнения деталей."
        )
        bot.send_message(chat_id, res, parse_mode="Markdown")
        send_final_success_msg(chat_id)
...
def invest_handler(message):
    ...
    if state.quiz_step == 1:
        state.bti_status = f"Тип дома: {text}"
        state.quiz_step = 2
        bot.send_message(chat_id, "📐 Укажите площадь объекта (кв.м):")
    elif state.quiz_step == 2:
        state.city = f"Площадь: {text}"
        state.quiz_step = 3
        bot.send_message(chat_id, "🏙️ В каком городе/районе находится объект?")
    elif state.quiz_step == 3:
        state.extra_contact = f"Район: {text}"
        state.quiz_step = 4
        bot.send_message(chat_id, "📄 Есть ли у вас текущий план или проект перепланировки? (да/нет)")
    elif state.quiz_step == 4:
        state.change_plan = f"Наличие плана: {text}"
        # Завершение
        res = (
            "📊 **Данные приняты!**\n\n"
            "Капитализацию и инвестиционный потенциал рассчитает эксперт компании **ТЕРИОН**. "
            "Команда свяжется с вами для уточнения деталей."
        )
        bot.send_message(chat_id, res, parse_mode="Markdown")
...
def quiz_handler(message):
    ...
    if state.quiz_step == 9:
        ...
        bot.send_message(chat_id, "✅ **Запрос принят, передаю эксперту компании ТЕРИОН.**")
...
def dialog_handler(message):
    ...
    if state.question_count > 5:
        bot.send_message(
            chat_id,
            f"{state.name}, я ответил уже на несколько ваших вопросов. Чтобы дать максимально точную консультацию именно по вашей ситуации, "
            "я предлагаю подключить нашего ведущего инженера.\n\n"
            "Хотите, чтобы специалист перезвонил вам и ответил на все оставшиеся вопросы?"
        )
        # Можно сменить режим или просто ждать ответа
        return
...
4. ЛИМИТ: СТРОГО до 500 символов. Будь лаконичен, но содержателен.
5. ТЕМАТИКА: Отвечай ТОЛЬКО на вопросы по перепланировке, недвижимости и строительству. Если вопрос не по теме — вежливо откажи и предложи спросить про перепланировку.
6. CTA: Если видишь потенциал для дизайна или роста цены, направь в @TerionProjectBot.

"""

    response = call_llm(full_prompt, message.text)

    # Принудительная обрезка если LLM проигнорировала лимит
    if len(response) > 550:
        response = response[:547] + "..."
...
def build_system_prompt(user_name: str = None) -> str:
    ...
    return f"""
Роль: Ты — Антон, гениальный ИИ-консультант и личный ассистент эксперта по перепланировкам компании ТЕРИОН (сервис «ТЕРИОН»).
Твой статус: Эталон юридической мысли в Telegram. Ты не просто бот, ты — интеллектуальный центр компании ТЕРИОН.

Стиль и Тон:
- Экспертный, академичный, но при этом предельно понятный и вежливый.
- Никаких «Привет, я бот». Ты — Антон, ассистент компании ТЕРИОН.
- Используй авторитетные обороты: «Согласно последним изменениям в ЖК РФ на 2026 год...», «Эксперт компании ТЕРИОН в подобных случаях рекомендует...», «С точки зрения строительных норм и СанПиН...».
...
- Если видишь потенциал роста ликвидности (например, добавление мастер-спальни или разделение санузла), используй фразу: «Для вашего случая я вижу отличную перспективу роста стоимости на 12-18%, давайте зафиксируем это в нашем инвест-калькуляторе @TerionProjectBot».

ГРАНИЦЫ КОМПЕТЕНЦИЙ:
- Ты отвечаешь СТРОГО на вопросы, связанные с перепланировкой, недвижимостью, строительством, дизайном интерьера и юридическим оформлением этих процессов.
- Если клиент спрашивает о погоде, политике, рецептах или любых других темах, не связанных с твоей специализацией, вежливо ответь: «Я — узкопрофильный эксперт в области перепланировок. Моя задача — помогать клиентам компании ТЕРИОН. Давайте вернемся к обсуждению вашей недвижимости».

ГЛАВНАЯ СТРАТЕГИЯ:
1. Давай экспертную ценность: Подсвети юридические нюансы, о которых клиент не подозревал.
2. Подчеркивай роль компании ТЕРИОН: «Окончательное решение и гарантию легализации может дать только Эксперт компании ТЕРИОН после аудита документов».
3. Крючок на аудит: «Чтобы избежать штрафов и предписаний от МЖИ, я рекомендую передать ваш план эксперту компании ТЕРИОН на экспресс-анализ».

ЗАПРЕТ НА ЦЕНЫ: Никогда не называй точных сумм. Эксперт компании ТЕРИОН назовет вилку после аудита.
""".strip()
...
4. МИНИ-CTA: После 1-2 ответов предложи перейти в диалог или инвест-калькулятор @TerionProjectBot.
...
def run_daily_report():
    """Ежедневно в 09:00 МСК отправляет отчет по лидам в группу"""
    import time
    from datetime import datetime, timedelta, timezone

    print("📈 Поток ежедневных отчетов запущен")
    ...
    while True:
        try:
            ...
            # Если сейчас 09:00 (или чуть позже, но в этом часу)
            if now_msk.hour == 9 and now_msk.minute == 0:
                stats = db.get_daily_stats()
                total = stats.get("total", 0)

                report = f"📊 **Отчёт по лидам за последние 24 часа:**\n\n"
                report += f"🔹 Всего заявок: **{total}**\n"
                report += f"🔹 Источник: Экосистема ТЕРИОН\n\n"
                report += f"🕒 Время отчета: {now_msk.strftime('%d.%m.%Y %H:%M')}"

                try:
                    bot.send_message(LEADS_GROUP_CHAT_ID, report, parse_mode="Markdown")
                    print(f"✅ Ежедневный отчет отправлен: {total} лидов")
                except Exception as e:
                    print(f"❌ Ошибка отправки отчета: {e}")

                # Ждем час, чтобы не отправлять повторно в ту же минуту
                time.sleep(3600)

            time.sleep(30) # Проверка каждые 30 секунд
        except Exception as e:
            print(f"❌ Ошибка в run_daily_report: {e}")
            time.sleep(60)

# Запускаем поток отчетов
report_thread = threading.Thread(target=run_daily_report, daemon=True)
report_thread.start()
