...
KNOWLEDGE_DIR = "lad_expert_kb/knowledge_base"
...
def show_privacy_consent(chat_id: int):
    ...
    text = (
        "👋 **Добро пожаловать в сервис «ЛАД: Согласование и Проектирование»!**\n\n"
        "Перед началом работы, пожалуйста, ознакомьтесь с условиями:\n"
        "1. Мы обрабатываем ваши данные согласно [Политике конфиденциальности](https://remodeling-expert.ru/privacy).\n"
        "2. Вы соглашаетесь на получение уведомлений о статусе вашей заявки.\n\n"
...
def send_checklist(chat_id: int):
    ...
    checklist_text = (
        "🎁 **Ваш подарок: Чек-лист документов для перепланировки**\n\n"
        "Пока наш эксперт изучает ваш запрос, проверьте, что из этого списка у вас уже есть на руках:\n\n"
        "1️⃣ **Техпаспорт БТИ** (поэтажный план и экспликация) — основной документ.\n"
        "2️⃣ **Выписка из ЕГРН** — подтверждает право собственности и отсутствие обременений.\n"
        "3️⃣ **Правоустанавливающие документы** (договор купли-продажи, дарения и т.д.).\n"
        "4️⃣ **Согласие всех собственников** — если у квартиры несколько владельцев.\n"
        "5️⃣ **Проект перепланировки** — если вы уже начали подготовку.\n\n"
        "⚠️ Если чего-то не хватает — не переживайте, мы поможем собрать недостающие документы!"
    )
...
def send_final_success_msg(chat_id: int):
    ...
    if is_working_hours():
        timing_info = "свяжется с вами в ближайшее время"
    else:
        timing_info = "свяжется с вами в рабочее время (с 10:00 завтра)"

    final_msg = (
        f"✅ Спасибо, {state.name or 'Юлия'}! Ваша заявка принята.\n\n"
        f"Команда «ЛАД: Согласование и Проектирование» {timing_info} по номеру {state.phone} "
        f"для обсуждения деталей и предварительного расчёта.\n\n"
        f"Вы также можете прислать фото плана или написать любую дополнительную информацию прямо здесь в чате — "
        f"я передам её специалисту."
    )
...
def expert_handler(message):
    ...
    if state.quiz_step == 1:
        ...
        state.quiz_step = 2
        bot.send_message(chat_id, "🏙️ Укажите ваш город и тип дома (панельный, кирпичный, монолит):")
    elif state.quiz_step == 2:
        state.city = text
        state.quiz_step = 3
        bot.send_message(chat_id, "🔢 На каком этаже находится объект и сколько всего этажей в доме? (например: 3/5)")
        ...
        res = (
            f"✅ **Ваш запрос принят!**\n\n"
            f"Предварительно — задача решаема, но есть нюансы по вашему запросу («{state.change_plan}»). "
            f"Я передал все детали нашему эксперту для подготовки краткого юридического заключения."
        )
...
def price_handler(message):
    ...
    if state.quiz_step == 1:
        ...
        res = (
            f"✅ **Данные получены!**\n\n"
            f"Предварительный расчет подготовлен. Итоговую смету с вами обсудит наш эксперт после уточнения деталей и изучения документов."
        )
        bot.send_message(chat_id, res, parse_mode="Markdown")
        send_final_success_msg(chat_id)
...
6. CTA: Если видишь потенциал для дизайна или роста цены, направь в @Lad_v_kvartire_bot.
...
- Если клиент спрашивает о погоде, политике, рецептах или любых других темах, не связанных с твоей специализацией, вежливо ответь: «Я — узкопрофильный эксперт в области перепланировок. Моя задача — помогать клиентам Юлии Владимировны Пархоменко. Давайте вернемся к обсуждению вашей недвижимости».
