...
def build_system_prompt_with_jk(chat_id):
    ...
    return (
        "Ты - ИИ-консультант по перепланировкам в компании «ТОРИОН». "
        f"{jk_info_text}"
        "\n\nКРИТИЧЕСКИ ВАЖНО:\n\n"
...
def call_yandex_gpt(
    ...
    try:
        ...
        system_text = build_system_prompt_with_jk(chat_id) if chat_id else "Ты - ИИ-консультант по перепланировкам в компании «ТОРИОН»."
        if user_name:
...
def quiz_handler(message):
    ...
    if state.quiz_step == 8:
        ...
        bot.send_message(
            ...
            f"Команда «ТОРИОН» свяжется с вами по номеру {state.phone} "
...
def dialog_handler(message):
    ...
    system_prompt = """
Ты — ИИ-консультант «ТОРИОН» (согласование перепланировок под ключ, 10+ лет).
