...
def show_privacy_consent(chat_id: int):
    ...
    jk_greeting = ""
    if state.source:
        jk_info = geo_db.get_jk_info(state.source)
        if jk_info:
            jk_greeting = f"🏠 Приветствую жителя <b>{jk_info['name']}</b>! Я знаком с конструктивом ваших домов…\n\n"
        else:
            jk_greeting = f"🏠 Приветствую! Вижу, вы из {state.source}. Рад помочь вам с перепланировкой.\n\n"
...
def save_lead_and_notify(user_id: int):
    ...
    # Проверка на ночную заявку
    night_prefix = "🌙 <b>Ночная заявка</b>\n" if not is_working_hours() else ""
    jk_prefix = f"🏢 <b>ЖК: {state.source}</b>\n" if state.source else ""

    lead_info = f"""
{night_prefix}{jk_prefix}📋 Новая заявка на перепланировку
...
# Групповые триггеры и Privacy Mode
...
def group_handler(message):
    ...
    keywords = [
        "перепланировка", "снос стены", "мокрая зона",
        "объединение лоджии", "проект", "узаконить"
    ]
    has_keyword = any(kw in text for kw in keywords)

    if is_mentioned or has_keyword:
        # Краткий ответ в группу
        bot.reply_to(
            message,
            "Вижу вопрос по перепланировке. Тема сложная, чтобы не засорять чат соседям, "
            "ответил вам подробно в личных сообщениях. Загляните туда."
        )

        # Подробный ответ в ЛС
        try:
            welcome_dm = (
                "Здравствуйте! Вы задали вопрос в группе. Я — эксперт ТЕРИОН.\n\n"
                "Перепланировка — это серьезный процесс, требующий учета конструктива дома. "
                "Чтобы я мог проконсультировать вас точнее, пожалуйста, выберите нужный раздел в меню или просто опишите вашу ситуацию."
            )
            bot.send_message(message.from_user.id, welcome_dm)

            # Предлагаем начать квиз или консультацию
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("📋 Начать опрос", callback_data="mode_quiz"))
            markup.add(types.InlineKeyboardButton("💬 Консультация", callback_data="mode_dialog"))
            bot.send_message(message.from_user.id, "С чего начнем?", reply_markup=markup)

        except Exception as e:
            # Если не удалось отправить в ЛС (пользователь не стартовал бота)
...
def start_handler(message):
    ...
    # Обработка Deep-Linking (ЖК-трекинг)
    payload = ""
    if len(message.text.split()) > 1:
        payload = message.text.split()[1]
        state.source = payload
        # Сохраняем источник в лид сразу, если он есть
        run_async(db.save_lead(user_id, source=payload))
...
def admin_handler(message):
    if message.chat.id != ADMIN_ID and message.chat.id != LEADS_GROUP_CHAT_ID:
        return

    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🔗 Список ссылок ЖК", callback_data="admin_jk_links"))
    markup.add(types.InlineKeyboardButton("➕ Создать ссылку ЖК", callback_data="admin_jk_create"))
    markup.add(types.InlineKeyboardButton("📊 Статистика ЖК", callback_data="admin_jk_stats"))

    bot.send_message(message.chat.id, "🏗 <b>Админ-панель ТЕРИОН</b>", reply_markup=markup, parse_mode="HTML")


@bot.callback_query_handler(func=lambda call: call.data.startswith("admin_"))
def admin_callback_handler(call):
    chat_id = call.message.chat.id
    if chat_id != ADMIN_ID and chat_id != LEADS_GROUP_CHAT_ID:
        return

    if call.data == "admin_jk_links":
        jks = geo_db.list_jks()
        text = "🔗 <b>Ссылки для ЖК:</b>\n\n"
        bot_username = bot.get_me().username
        for jk in jks:
            text += f"🏠 {jk['name']}:\n<code>https://t.me/{bot_username}?start={jk['code']}</code>\n\n"

        bot.send_message(chat_id, text, parse_mode="HTML")

    elif call.data == "admin_jk_create":
        bot.send_message(chat_id, "Введите название нового ЖК для генерации ссылки (например: JK_Lefortovo):")
        # В MVP будем использовать состояние для ожидания ввода
        # Но так как бот синхронный и стейты простые, используем register_next_step_handler
        bot.register_next_step_handler(call.message, process_admin_jk_creation)

    elif call.data == "admin_jk_stats":
        stats = run_async(db.get_jk_stats())
        if not stats:
            bot.send_message(chat_id, "📊 Статистика пока пуста.")
            return

        text = "📊 <b>Статистика по ЖК:</b>\n\n"
        for row in stats:
            text += f"🏢 {row['source']}: {row['count']} лидов\n"
...
def process_admin_jk_creation(message):
    ...
    # Генерируем код (латиница без пробелов)
    # В реальности тут нужна транслитерация, но для MVP просто уберем пробелы
    jk_code = jk_name.replace(" ", "_")

    geo_db.add_jk(jk_code, jk_name)

    bot_username = bot.get_me().username
    url = f"https://t.me/{bot_username}?start={jk_code}"

    bot.send_message(
        chat_id,
        f"✅ Ссылка для ЖК <b>{jk_name}</b> создана!\n\nURL: <code>{url}</code>",
        parse_mode="HTML"
    )
...
def build_system_prompt(chat_id: int):
    ...
    if state.source:
        jk_info = geo_db.get_jk_info(state.source)
        if jk_info:
            jk_context = f"\nКЛИЕНТ ИЗ {jk_info['name']}. Особенности дома: {jk_info['features']}. Правила: {jk_info['rules']}\n"
        else:
            jk_context = f"\nКЛИЕНТ ИЗ {state.source}.\n"
