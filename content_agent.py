...
class ContentAgent:
    ...
    def _build_prompt(self, post_type, theme=None):
        ...
        prompts = {
            'news': f"""
Создай новостной пост для Telegram-канала по перепланировкам квартир в Москве.
...
""",
            'fact': f"""
Создай пост с интересным фактом/мифом для Telegram-канала по перепланировкам квартир в Москве.

Контекст сезона: {season}{theme_note}

Требования:
- Интересный факт или развенчание распространённого мифа
- 150–250 слов, увлекательно, с примерами
- Объяснение почему это важно знать
- CTA к боту @Parkhovenko_i_kompaniya_bot для консультации

Формат ответа:
[Заголовок факта] (опционально)

[Основной текст с фактом/мифом]
...
""",
            'seasonal': f"""
Создай сезонный пост для Telegram-канала по перепланировкам квартир в Москве.

Контекст сезона: {season}{theme_note}

Требования:
- Привязка к текущему сезону и актуальным проблемам жильцов
- 150–250 слов, по-человечески, с личной ноткой
- Связка с темой перепланировок и рисками
- CTA к боту @Parkhovenko_i_kompaniya_bot

Формат ответа:
[Сезонный зацеп или наблюдение]

[Связка с перепланировками]
...
""",
            'case': f"""
Создай пост с кейсом «было/стало» для Telegram-канала по перепланировкам квартир в Москве.

Требования:
- Реальный или типичный кейс от команды «Пархоменко и компания»
- 180–300 слов, с конкретными деталями
- Показать проблему, решение и результат
- CTA к боту @Parkhovenko_i_kompaniya_bot для подобной ситуации{theme_note}
...
""",
            'offer': f"""
Создай мягкий продающий пост для Telegram-канала по перепланировкам квартир в Москве.

Требования:
- Акцент на преимуществах работы с профессионалами
- 150–200 слов, доверительно, без агрессивных продаж
- Подчёркивание гарантий и комплексного подхода
- CTA к боту @Parkhovenko_i_kompaniya_bot для консультации{theme_note}

Формат ответа:
[Заголовок предложения] (опционально)
