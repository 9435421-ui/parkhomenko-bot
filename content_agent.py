...
class ContentAgent:
    ...
    async def _call_yandex_gpt(self, user_prompt: str) -> str:
        """Асинхронный вызов LLM API через aiohttp (OpenAI-совместимый для RouterAI)"""
...
Каждый пост должен вести к действию: переходу в @Parkhovenko_i_kompaniya_bot."""
        ...
        # Если используется RouterAI endpoint, используем формат OpenAI
...
- Структурируй информацию логически"""
        ...
        prompts = {
            'экспертиза': f"""Создай мощный экспертный пост, который продаёт услуги TERION.

Реальные кейсы и термины (ОБЯЗАТЕЛЬНО внедрить в текст): {expert_block}

Контекст: {season}{theme_note}{max_instructions}

ЦЕЛЬ: Доказать, что самостоятельное согласование или игнорирование норм — это путь к огромным штрафам и судам.

СТРУКТУРА:
1) 💡 Провокационный или гипер-экспертный заголовок
2) 📝 Разбор конкретной ситуации (кейса) через боли: МЖИ, несущие конструкции, риск обрушения или невозможность продажи.
3) 🎯 Решение от TERION и четкий призыв: {CTA_TEXT}{vk_hashtags}""",
            'живой': f"""Создай вовлекающий «живой» пост для канала ТЕРИОН.

Контекст сезона: {season}{theme_note}{max_instructions}

ЦЕЛЬ: Создать доверие через человечный подход и понимание текущих забот собственников.

СТРУКТУРА:
1) 🎬 Живой зацеп: что сейчас происходит в ЖК Москвы (ремонты, ключи, проверки).
2) 💬 Дружеский совет эксперта: на чем нельзя экономить и почему бумажная работа важнее дизайна.
3) 👉 Переход к действию: {CTA_TEXT}{vk_hashtags}""",
