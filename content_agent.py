import aiohttp
import os
from datetime import datetime, timedelta
import json
from image_agent import ImageAgent

class ContentAgent:
    def __init__(self, api_key: str, model_uri: str):
        self.folder_id = os.getenv("FOLDER_ID")
        self.api_key = api_key
        self.model_uri = model_uri
        self.image_agent = ImageAgent()
        self.endpoint = "https://llm.api.cloud.yandex.net/foundationModels/v1/completion"
        self.brief_content = self._load_brief()

        # Единый список типов постов
        self.post_types = ['news', 'fact', 'case', 'offer', 'seasonal']

        # Шаблоны поздравлений с днем рождения (без упоминания услуг/перепланировок)
        self.birthday_templates = [
            "Поздравляем вас с днем рождения! Пусть этот день будет наполнен радостью, теплом близких и приятными сюрпризами. Желаем крепкого здоровья, душевного равновесия и исполнения самых заветных желаний.",
            "С днем рождения! Пусть этот особенный день принесет вам море улыбок, тепла от родных и друзей, а также исполнение всех мечтаний. Желаем здоровья, счастья и благополучия на каждый день.",
            "Поздравляем с днем рождения! Пусть этот день будет ярким и незабываемым, наполненным любовью близких и приятными моментами. Желаем крепкого здоровья, семейного тепла и достижения всех поставленных целей.",
            "С днем рождения! Пусть этот праздник принесет вам заряд положительных эмоций, теплые объятия родных и исполнение желаний. Желаем здоровья, счастья и благополучия в вашей жизни.",
            "Поздравляем с днем рождения! Пусть этот день будет особенным, наполненным радостью, теплом и заботой близких. Желаем крепкого здоровья, душевного комфорта и достижения всех мечтаний.",
            "С днем рождения! Пусть этот праздник станет началом новых радостных событий в вашей жизни. Желаем здоровья, счастья, семейного тепла и исполнения самых сокровенных желаний.",
            "Поздравляем с днем рождения! Пусть этот день будет ярким и незабываемым, а каждый новый день приносит новые возможности и радости. Желаем крепкого здоровья и благополучия.",
            "С днем рождения! Желаем вам тепла от близких, радости от маленьких побед и исполнения мечтаний. Пусть этот день станет одним из самых счастливых в вашей жизни."
        ]

    def _load_brief(self):
        """Загружает базу знаний"""
        try:
            with open("BRIEF_pereplanirovki.md", "r", encoding="utf-8") as f:
                return f.read()
        except FileNotFoundError:
            return "База знаний не найдена. Используйте общие знания по перепланировкам."

    async def generate_post_text(self, plan_item):
        """
        Сгенерировать текст поста в зависимости от plan_item.type

        Args:
            plan_item: Словарь с полями type, theme и др.

        Returns:
            dict: {'title': str, 'body': str, 'cta': str}
        """
        post_type = plan_item.get('type')
        theme = plan_item.get('theme')

        if post_type not in self.post_types:
            # Фолбэк для неизвестных типов
            post_type = 'fact'

        prompt = self._build_prompt(post_type, theme)
        text = await self._call_yandex_gpt(prompt)

        # Парсим ответ: заголовок (опционально), текст, CTA
        title, body, cta = self._parse_response(text)

        return {
            'title': title,
            'body': body,
            'cta': cta
        }

    async def generate_posts(self, count=7, post_types=None, theme=None):
        """
        Генерирует N постов (асинхронный)
        post_types: {'news': 2, 'fact': 3, 'seasonal': 1, 'case': 1}
        theme: опциональная тема недели (например: "новый год и зимние перепланировки")
        """
        if post_types is None:
            # Новое распределение без seasonal: 3 fact, 2 case, 1 news, 1 offer
            post_types = {
                'fact': 3,
                'case': 2,
                'news': 1,
                'offer': 1,
            }

        posts = []
        start_date = datetime.now() + timedelta(days=1)
        start_date = start_date.replace(hour=10, minute=0, second=0)

        for post_type, num in post_types.items():
            for i in range(num):
                # Используем новый метод генерации
                plan_item = {'type': post_type, 'theme': theme}
                text_data = await self.generate_post_text(plan_item)

                post = {
                    'type': post_type,
                    'title': text_data['title'],
                    'body': text_data['body'],
                    'cta': text_data['cta'],
                    'publish_date': start_date + timedelta(days=len(posts)),
                    'image_prompt': self.image_agent.build_image_prompt(post_type, text_data['body'])
                }
                posts.append(post)

        return posts

    def _build_prompt(self, post_type, theme=None):
        """Формирует промпт для конкретного типа поста"""
        season = self._get_season_context()
        theme_note = f"\nУчитывай тему недели: {theme}" if theme else ""

        prompts = {
            "news": f"""
Создай пост о изменениях в правилах перепланировок, работе БТИ, Мосжилинспекции, банков и ипотеки в Москве/МО.
{theme_note}
Требования:
- Фокус только на перепланировках и их согласовании.
- Примеры тем: банк ужесточил политику по кредитам для квартир с неузаконенной перепланировкой; изменения в правилах согласования перепланировок; типовой кейс отказа из-за перепланировки.
- Не использовать общий «инвест-новости про недвижимость».
- Стиль: дорогой консалтинг.
- Формат: заголовок-крючок, суть изменений, конкретный риск для сделки, CTA.
- CTA: «Если у вас уже есть перепланировка или вы только планируете, пройдите квиз в боте Юлии Пархоменко и получите предварительную оценку рисков по вашей квартире: @Parkhovenko_i_kompaniya_bot»
""",
            "fact": f"""
Развенчай миф о перепланировке, который мешает инвесторам или дизайнерам зарабатывать.
{theme_note}
Требования:
- Никакой погоды, бытовых советов, утепления окон или балконов.
- Пример: миф о том, что «дизайнерский ремонт» всегда повышает цену (на самом деле незаконная планировка её снижает).
- Используй логику: «проект живёт в интерьере, но отсутствует в документах».
""",
            "seasonal": f"""
Создай пост о стратегии работы с недвижимостью в текущий деловой период.
{theme_note}
Требования:
- Вместо погоды пиши про деловой сезон: завершение сделок, старт новых инвест‑циклов, подготовку объекта к продаже.
- Акцент на поддержке и уверенности для дизайнера и инвестора.
- Покажи, как вовремя проверить и согласовать перепланировку, чтобы не тормозить сделки.
""",
            "case": f"""
Опиши кейс в стиле Юлии Пархоменко.
{theme_note}
Требования:
- Герой: инвестор или дизайнер.
- Сюжет: эффектная перепланировка в Москве, которая столкнулась с реальностью при продаже или оценке банком.
- Результат: как своевременное согласование спасло сделку или увеличило стоимость объекта.
""",
            "offer": f"""
Создай пост‑предложение о партнёрстве для дизайнеров и инвесторов.
{theme_note}
Требования:
- Позиция: мы не контролёры, мы — часть команды, которая удерживает проект в реальности и документах.
- Оффер: сделать проект «подтверждённым» и защищённым — чтобы дизайн и документы совпадали.
- Обязательно мягкий призыв написать в @Parkhovenko_i_kompaniya_bot.
"""
        }

        return prompts.get(post_type, prompts['fact'])

    def _get_season_context(self) -> str:
        """Вместо погоды возвращает бизнес-фокус из эталонных вариантов."""
        return (
            "Фокус: перепланировка как живой актив. "
            "Риск: проект живёт только в интерьере, но не в документах. "
            "Последствие: пауза до первой сделки, проверка банка или отказ в ипотеке."
        )

    async def _call_yandex_gpt(self, user_prompt, mode="default", context=""):
        """Вызов YandexGPT API (асинхронный)"""

        if mode == "greeting":
            system_prompt = """
Ты пишешь короткие личные поздравления.

Твоя задача — писать тёплые человеческие тексты без продаж и без упоминания услуг, компаний, ботов, рекламы, акций и скидок.
Строго запрещено упоминать перепланировку, ремонт, юридические услуги, консультации, кредиты, долги, бизнес, команду экспертов, Telegram-ботов и любые предложения «обратиться», «заказать», «получить консультацию».

Пиши простым дружелюбным языком, как человек человеку.
Не добавляй ссылки, хэштеги, GIF, эмодзи кроме указанного в формате.
"""
        else:
            system_prompt = f"""
Ты — Юлия Пархоменко и её команда. Твой фокус — сложные кейсы по перепланировкам и согласованию, где на кону деньги, сделки и репутация.
Даже если где-то в контексте встречаются рассказы про зиму, балконы, утепление и погоду — игнорируй их. Пиши только про деньги, сделки, инвест‑логику и репутацию.

ТВОИ КЛИЕНТЫ:
- инвесторы, которые делят объект на лоты / студии, делают антресоли, меняют функционал помещений;
- дизайнеры, которые ведут проект и не хотят, чтобы красиво нарисованный интерьер «застрял» в БТИ;
- собственники, у которых уже есть объект с нестандартной планировкой и впереди сделка или проверка.

ЖЁСТКИЕ ТАБУ (ЗАПРЕЩЕНО):
- Любые тексты про погоду, время года и быт: «зима», «холод», «снег», «похолодало», «промерзающие окна», «сосульки», «отключение отопления», «лето», «дожди» и т.п.
- Темы «утепление окон/балконов/лоджий» как основной сюжет. Допустимо только вскользь, если это напрямую связано с инвест‑сделкой.
- Стартовые клише: «Многие считают», «С наступлением…», «Важно знать», «Однако это не так», «На первый взгляд».
- Канцелярит: «процедура», «нормативы», «согласно постановлению», «в соответствии с действующим законодательством». Говори человеческим языком.

ФОКУС СМЫСЛА:
1. Проблема всегда выражается через деньги и сделки:
   - сорванная сделка (ипотека не проходит, банк разворачивает покупателя);
   - падение ликвидности объекта (цена минус 10–30%);
   - риски для инвест‑модели (деление на студии не работает юридически).
2. Объект = актив. Перепланировка — инструмент:
   - либо увеличения доходности (деление, изменение функционала),
   - либо источник риска (если «живёт только в интерьере, а не в документах»).
3. Голос: партнёр по сделке, а не контролёр.
   - «Мы удерживаем проект в реальности и документах»,
   - а не «мы проверяем вас на соответствие нормам».

СТРУКТУРА ПОСТА (ОБЯЗАТЕЛЬНАЯ):
1) Заголовок‑крючок:
   - фиксирует проблему в деньгах/сделке.
   - Примеры форматов:
     - «Инвест‑проект на паузе. До первой сделки.»
     - «Студии на плане есть. В документах — нет.»
2) Ситуация с героем:
   - конкретный герой: дизайнер, инвестор, собственник.
   - что он сделал или задумал: деление на студии, перенос кухни, объединение квартир, перепрофилирование помещения и т.п.
   - Никаких абстрактных «собственников», всегда живой персонаж.
3) Последствия:
   - в деньгах и процессах: сделка не проходит, банк разворачивает, покупатель уходит, объект зависает на рынке, стоимость падает.
   - чётко: что именно пошло не так и почему.
4) Вывод + мягкий оффер:
   - короткий вывод: что нужно было учесть с точки зрения согласований и документов.
   - CTA в формате:
     - «Если вы не хотите, чтобы проект жил только в интерьере, а не в документах — напишите в @Parkhovenko_i_kompaniya_bot.»
     - «Перед тем как выводить объект на рынок, дайте проект на проверку в @Parkhovenko_i_kompaniya_bot.»

ОСОБЫЕ ПРАВИЛА ДЛЯ ТЕМЫ «ИНВЕСТ‑ПЕРЕПЛАНИРОВКИ»:
- Если тема или запрос содержит слова «инвест», «инвест‑перепланировки», «деление на студии», «деление на лоты», «инвест‑проект», 
  ПИШИ ТОЛЬКО про:
  - стратегию (как задумали зарабатывать на объекте),
  - юридические риски для этой стратегии,
  - последствия для сделки (ипотека, проверка, выход из проекта).
- Запрещено уходить в быт (тепло/холод, комфорт, «чтобы не дуло из окон»).

СТИЛЬ:
- Короткие абзацы, без «простыней».
- Никаких «страшилок ради страшилок» — только реалистичные сценарии: банк, Росреестр, покупатель, проверка.
- Пиши так, как будто объясняешь это дизайнеру или инвестору, который сейчас в процессе проекта, а не обычному обывателю.

ВАЖНО: РАЗНООБРАЗИЕ И УНИКАЛЬНОСТЬ:
- Каждый пост должен быть уникальным. Пиши каждый раз в новом стиле, избегай самоповторов.
- Не используй одинаковые формулировки, клише и структуры в разных постах.
- Меняй подход: где-то начни с вопроса, где-то с факта, где-то с истории.
- Избегай повторения одних и тех же примеров и кейсов.

Ты всегда пишешь от лица Юлии Пархоменко / её команды.
В конце каждого поста обязательно есть мягкий оффер на бота @Parkhovenko_i_kompaniya_bot.
Темы для генерации: {user_prompt}
"""

        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Api-Key {self.api_key}",
            "x-folder-id": self.folder_id
        }

        # Build messages with optional context
        messages = [{"role": "system", "text": system_prompt}]
        if context:
            messages.append({"role": "user", "text": context})
        messages.append({"role": "user", "text": user_prompt})

        payload = {
            "modelUri": self.model_uri,
            "completionOptions": {
                "stream": False,
                "temperature": 0.7,
                "maxTokens": 2000
            },
            "messages": messages
        }

        async with aiohttp.ClientSession() as session:
            async with session.post(self.endpoint, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=30)) as response:
                response.raise_for_status()
                result = await response.json()
                return result['result']['alternatives'][0]['message']['text']

    def _parse_response(self, text):
        """Парсит ответ LLM на title, body, cta"""
        lines = text.strip().split('\n')

        # Ищем CTA (строки с 👉 или "CTA:")
        cta_line = None
        for i, line in enumerate(lines):
            if '👉' in line or 'CTA:' in line.upper():
                cta_line = i
                break

        if cta_line:
            cta = '\n'.join(lines[cta_line:]).strip()
            body_lines = lines[:cta_line]
        else:
            cta = ""
            body_lines = lines

        # Первая строка может быть заголовком, если она короткая
        title = ""
        if body_lines and len(body_lines[0]) < 100:
            title = body_lines[0].strip('#').strip()
            body_lines = body_lines[1:]

        body = '\n'.join(body_lines).strip()

        return title, body, cta

    def _sanitize_greeting(self, text: str) -> str:
        """Убирает слишком продающие поздравления (страховка)"""
        import re
        banned = [
            "ремонт", "перепланиров", "услуг", "консультац", "бот",
            "телеграм", "команда экспертов", "скидк", "предложение",
            "закажите", "обратитесь", "юридическ", "кредит", "долг"
        ]
        lower = text.lower()
        # Разбиваем текст на слова (по пробелам и знакам препинания)
        words = re.findall(r'\b\w+\b', lower)
        # Проверяем наличие запрещенных слов как целых слов
        if any(banned_word in words for banned_word in banned):
            # Фолбэк — нейтральное поздравление без продаж
            return (
                "Пусть этот день будет для вас тёплым и радостным. "
                "Желаем здоровья, душевного комфорта, уюта дома и людей рядом, "
                "с которыми хочется делиться хорошими моментами."
            )
        return text

    async def generate_greeting_post(self, person_name, date, occasion='день рождения'):
        """
        Генерирует персональный поздравительный пост

        Args:
            person_name: Имя человека (может быть пустым)
            date: Дата в формате DD.MM или DD.MM.YYYY
            occasion: Повод для поздравления ('день рождения', 'Новый год', etc.)

        Returns:
            dict: {'title': str, 'body': str, 'cta': str, 'type': 'поздравление'}
        """
        display_name = person_name if person_name else "наш подписчик"

        prompt = f"""
Создай короткое искреннее поздравление для личного сообщения в Telegram.

Повод: {occasion}
Имя: {display_name}
Дата: {date}

Требования:
- Тёплое персональное поздравление (60–100 слов)
- Пожелания здоровья, радости, душевного комфорта, уюта дома
- Простой дружелюбный язык, без высокопарного канцелярита
- Строго запрещено упоминать услуги, ремонт, перепланировку, консультации, ботов, компании, акции, скидки и любые продажи.
- Не добавляй CTA, ссылки, хэштеги, GIF и призывы что-то заказать или узнать.

Формат ответа:
🎂 {display_name}

[Поздравление с пожеланиями]
"""

        text = await self._call_yandex_gpt(prompt, mode="greeting")
        text = self._sanitize_greeting(text)

        # Для поздравления title можно не использовать, всё в body
        title = ""
        body = text.strip()
        cta = ""  # никаких CTA в поздравлении

        return {
            'type': 'поздравление',
            'title': title,
            'body': body,
            'cta': cta
        }

    def generate_birthday_congrats_template(self, person_name: str | None, date: str):
        """
        Генерирует поздравление с днем рождения из шаблонов (без ИИ)

        Args:
            person_name: Имя человека или None
            date: Дата в формате DD.MM или DD.MM.YYYY

        Returns:
            dict: {'title': str, 'body': str, 'cta': str, 'type': 'поздравление'}
        """
        import random

        # Выбираем случайный шаблон
        congrats_text = random.choice(self.birthday_templates)

        # Формируем имя для поздравления
        display_name = person_name if person_name else "наш подписчик"

        # Формируем тело сообщения
        body = f"🎂 {display_name}\n\n{congrats_text}"

        return {
            'type': 'поздравление',
            'title': '',
            'body': body,
            'cta': ''
        }

    async def generate_welcome_post(self, person_name: str | None = None):
        """
        Генерирует приветственное сообщение-визитку для потенциального клиента.

        Args:
            person_name: Имя человека или None

        Returns:
            dict: {'title': str, 'body': str, 'cta': str, 'type': 'приветствие'}
        """
        display_name = person_name if person_name else "новый подписчик"

        prompt = f"""
Создай приветственное сообщение для Telegram-канала по перепланировкам.

Адресат: {display_name}

Требования:
- 80–120 слов
- Кратко представить канал и пользу: помощь с вопросами перепланировки, рисками, узакониванием
- 1–2 простых примера ситуаций, когда к нам можно обратиться (перенос стен, объединение кухни и комнаты, перепланировка с мокрыми зонами)
- Один аккуратный CTA к боту @Parkhovenko_i_kompaniya_bot
- Спокойный, дружелюбный тон

Формат ответа:
👋 {display_name}

[Текст приветствия 2–3 абзаца с CTA в конце]
"""

        text = await self._call_yandex_gpt(prompt, mode="default")  # обычный продающий режим
        title, body, cta = self._parse_response(text)

        return {
            'type': 'приветствие',
            'title': title,
            'body': body,
            'cta': cta
        }
