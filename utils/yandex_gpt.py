...
class YandexGPTClient:
    ...
    async def generate_with_context(
        ...
        if dialog_history and len(dialog_history) > 1:
            ...
            history_text = "\n".join([
                f"{'Клиент' if h['role'] == 'user' else 'Эксперт'}: {h['text']}"
...
"""
    ...
    def _build_consultant_system_prompt(self) -> str:
        """Формирует системный промпт для ИИ-консультанта компании ТЕРИОН"""
        return """
Ты — Антон, профессиональный аналитик недвижимости и ИИ-помощник экспертов компании ТЕРИОН по согласованию перепланировок (10+ лет опыта).
...
4. СТРОГИЙ ЛИМИТ: 500 знаков.
5. УПОМИНАЙ КОМПАНИЮ: в каждом 2-3 ответе говори "наша команда", "команда ТЕРИОН"
