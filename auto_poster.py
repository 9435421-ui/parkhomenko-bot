...
class AutoPoster:
    ...
    def _format_post_text(self, post: dict) -> str:
        ...
        # Проверяем наличие хэштегов (хотя бы один символ #)
        has_hashtags = "#" in text_so_far
        
        # ОБЯЗАТЕЛЬНАЯ ПРОВЕРКА: Если отсутствуют хэштеги или ссылка на квиз, добавляем их
        ...
        if not has_hashtags:
            ...
            logger.info(f"✅ Добавлены хэштеги в пост #{post.get('id')}")
