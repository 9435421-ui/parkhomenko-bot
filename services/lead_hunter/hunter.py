...
class LeadHunter:
    ...
    async def _analyze_intent(self, text: str) -> dict:
        ...
        system_prompt = (
            ...
            "\n\n"
            "СТРОГАЯ ФИЛЬТРАЦИЯ — ОБЯЗАТЕЛЬНО ОТСЕИВАТЬ:\n"
            "- Образовательные статьи, новости ЖКХ, политические программы, генеалогические данные, общие новости\n"
            "- Посты от владельцев групп/каналов (broadcast posts), а не вопросы от частных лиц\n"
            "- Генеалогия, история, архивы, РГАДА, архивные документы\n"
            "- Политика, партийные съезды, съезд партии\n"
            "- Спорт: волейбольные турниры, футбол\n"
            "- Реклама мебели, вакансии, аренда\n"
            "- Если текст не содержит вопроса от частного лица или владельца бизнеса — is_lead: false\n"
            "\n"
            "ПРИОРИТЕТ (is_lead: true) ТОЛЬКО ЕСЛИ:\n"
            "- Это вопрос от частного лица или владельца бизнеса\n"
            "- Содержит слова: 'можно ли', 'как узаконить', 'цена проекта', 'проблемы с МЖИ', 'штраф за стену'\n"
            "- Явно относится к Москве или Московской области (МСК, МО, Подмосковье)\n"
            "- ИЛИ это приоритетный ЖК из базы данных (Зиларт, Династия, Символ и др.)\n"
            "\n"
            "ГЕО-ПРИВЯЗКА:\n"
            "- Игнорируй любые сообщения, не относящиеся к Москве и Московской области\n"
