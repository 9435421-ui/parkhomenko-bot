...
class LeadHunter:
    """Автономный поиск и привлечение клиентов (Lead Hunter) v3.0 (Unified & Optimized)"""
    ...
    async def hunt(self):
        ...
        logger.info("🏹 LeadHunter v3.0: начало охоты...")
        ...
        if not all_posts:
            ...
            for src in new_sources:
                await main_db.add_target_resource(src["source_type"], src["link"], title=src["title"], status="pending")
            return
    ...
    async def _send_lead_card_to_group(self, lead, lead_id, profile_url, post_url, card_header):
        ...
        buttons = [
            [InlineKeyboardButton(text="🔗 Пост", url=post_url)],
        ...
        if profile_url.startswith("http"):
            buttons[0].append(InlineKeyboardButton(text="👤 Профиль", url=profile_url))
    ...
    async def _send_raw_leads_file_to_group(self, all_posts):
        # Оставляем как было или упрощаем
        pass
