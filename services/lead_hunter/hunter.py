...
class LeadHunter:
    ...
    async def _generate_sales_reply(
        ...
        system_prompt = (
            "Ты — эксперт компании TERION по согласованию перепланировок в Москве. "
            "Пишешь ответ в публичный Telegram-чат жильцов. "
            "Правила: не начинай с 'Здравствуйте', не используй маркетинговые клише, "
        ...
        user_prompt = (
            f"Чат: {zhk_hint}\n"
            f"Стадия боли: {pain_stage}\n"
            f"Сообщение клиента: \"{(post_text or '')[:400]}\"\n"
            f"Интент: {intent}\n"
            f"Контекст: {context_summary}\n\n"
            f"Задача ответа: {script_hint}\n\n"
            "Напиши только текст ответа, без объяснений и заголовков."
        )
